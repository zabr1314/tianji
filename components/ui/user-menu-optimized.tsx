...
// 优化：分离统计卡片组件
const StatsCard = memo(({ points, recordsCount, isLoading }: {
  points: number | null
...
// 优化：分离菜单项组件
const MenuLink = memo(({ href, icon: Icon, children, badge }: {
  href: string
  icon: any
...
export const UserMenuOptimized = memo(({ user, onSignOut }: UserMenuProps) => {
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-64" align="end">
        ...
        <MenuLink 
          href="/history" 
          icon={History}
          badge={!isLoading && recordsCount !== null && recordsCount > 0 ? recordsCount : undefined}
        >
          历史记录
        </MenuLink>
...
UserMenuOptimized.displayName = 'UserMenuOptimized'

// 为了向后兼容，导出一个默认的UserMenu
export const UserMenu = UserMenuOptimized